...
public class ButtonBinder {
  ...
  */
  ...
  /**
   * Returns a trigger that is active when the specified analog input is above 0.5 where 1 is fully active.
   * 
   * @param joystick The GenericHID object that has the button (e.g. Joystick, KinectStick, etc)
   * @param buttonNumber The button number (see {@link GenericHID#getRawButton(int) }
  */
  public static Trigger bindAxis(GenericHID joystick, int buttonNumber) {
    ...
    return new Trigger(
      () -> joystick.getRawAxis(buttonNumber) >= .5
    );
  }
  ...
  */
   ...
   * @param button PS4Controller Axis enum{@link PS4Controller.Axis}.
  */
